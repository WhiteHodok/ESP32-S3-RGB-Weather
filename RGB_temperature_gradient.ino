...
void setPixelColor(int temp) {
  if (temp == 0) {
    ...
    // Для отрицательных температур (от -25°C до 0°C): плавный переход от синего к зелёному
    float ratio = (float)(temp - minTemp) / (0 - minTemp); // Нормализация в диапазоне -25-0
