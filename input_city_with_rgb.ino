...
void setup() {
  ...
  Serial.println("Введите название города (на английском, например: Moscow, Omsk, Sochi):");
}

void loop() {
  if (Serial.available() > 0) {
    String city = Serial.readStringUntil('\n'); // Читаем название города
    city.trim(); // Убираем лишние пробелы и символы новой строки

    if (city.length() > 0) {
      getTemperature(city); // Запрашиваем температуру для введённого города
    } else {
      Serial.println("Ошибка: Название города не может быть пустым.");
    }
  }
}
...
void getTemperature(String city) {
  if ((WiFi.status() == WL_CONNECTED)) { // Проверка статуса подключения
    ...
    String url = "https://wttr.in/" + city + "?format=%t"; // Формируем URL с городом
    http.begin(url); // Указываем URL
    int httpCode = http.GET(); // Отправляем запрос
    
    if (httpCode == 200) { // Проверяем код ответа (200 = успешно)
      ...
      if (temperatureEnd != -1) { // Если найдены символы "°C"
        ...
        Serial.println("Ошибка: Город не найден или данные о температуре недоступны.");
      }
    } else {
      Serial.println("Ошибка: Город не найден или сервер недоступен.");
    }
  ...
  Serial.println("Следующий запрос будет выполнен через 1 минуту...");
